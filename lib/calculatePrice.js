export function calculatePrice (data = [], priceAccessor, quantityAccessor, discountAccessor) {
    ...
    return (data.reduce((total, item) => total += (price(item) - discount(item) || 0) * quantity(item), 0 )).toFixed(2)
...
export function calculateTotal (subTotal, vat, discount) {
    return (parseFloat(subTotal) + parseFloat(vat) - parseFloat(discount)).toFixed(2)
}